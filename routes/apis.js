...
router.post('/create',(req,res)=>{
    try {
        ...
        if (!name||!password||!email||!mobile) res.status(200).json('Invalid Input');
        else {
            bcrypt.genSalt(10,(err,salt)=>{
                ...
                bcrypt.hash(password,salt,function(err,hash){
                    ...
                    // console.log(mob==NaN)
                    if (isNaN(parseInt(mobile))) res.status(200).json('Invalid Mobile Number')
                    else {
                        sql = `select * from employees where name='${name}' or email='${email}' or mobile=${parseInt(mobile)};`
                        console.log(sql)
                        ...
                        bcrypt.genSalt(10,(err,salt)=>{
                            ...
                            bcrypt.hash(password,salt,function(err,hash){
                                ...
                                if (isNaN(parseInt(mobile))) res.status(200).json('Invalid Mobile Number')
                                    ...
                                    con.query(`select * from employees where name='${name}' or email='${email}' or mobile=${parseInt(mobile)};`,(err,result,fields)=>{
                                        if (result.length === 0) {
                                            if (mobile.length===10 && email.includes('@') && password.length===8) {
                                                con.query(`insert into employees (name,password,mobile,email,admin) values ('${name}','${hash}',${mobile},'${email}',${true})`,(err,result)=>{
                                                    if (err) {res.status(400).json('Bad Request');throw err; }
                                                    else res.status(200).json('Root User added successfully')
                                                });
                                            } else res.status(200).json('Invalid Input')
                                        } else res.status(200).json('User already exists')
                                    })
